angular.module('2017Apps').controller('UserController', ['$window', '$rootScope', '$state', 'UserService', 'AccountService', 'AlertService', function ($window, $rootScope, $state, UserService, AccountService, AlertService) {
    var self = this;
    self.avaliableStore = ['00', '01', '02', '18'];
    self.currentStore = '18';
            ...
            FCMPlugin.getToken(function (deviceToken) {
                ...
                UserService.login(self.user, function (data) {
                    if (data.error)
                        ...
                        if (data.loginUser) {
                            for (var i = 0; i < data.loginUser.accounts.length; i++) {
                                if (data.loginUser.accounts[i].storeId == currentStore) {
                                    $rootScope.account = data.loginUser.accounts[i];
                                    $rootScope.role = data.loginUser.accounts[i].role;
                                 }
                            }
                            $rootScope.userId = data.loginUser._id;
                            $rootScope.username = data.loginUser.username;                
                        }
                        init();
                    }
                });
